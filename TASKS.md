...
- [X] **T-102** `P0` `M` `dados` Estender o envelope PanelResponse para as 12 formas e gerar o JSON Schema
...
- [X] **T-113** `P0` `L` `dados` Escrever as 21 métricas do Anexo B no catálogo
...
- [X] **T-117.2** `P0` `M` `auditoria` Declarar a fonte dos dois painéis de barras cujas medidas não existem em 10.1
...
- [X] **T-140.2** `P0` `M` `dados` Repartir com perfil sazonal para a fatia de uma dimensão variar entre meses
...
- [X] **T-140.3** `P0` `S` `dados` Dar a modalidade um custo por cabeça próprio, para a fatia dela diferir entre medidas
  · **Aceite:** A fatia anual do remoto na folha difere da fatia dele no quadro em mais de um ponto percentual, e o mesmo vale para presencial; os totais anuais por área, por entidade e o total de R$ 186 mi da folha continuam idênticos; o controle negativo de T-140 segue reprovando nas quatro dimensões já cobertas.
  · **PRD:** Anexo D achados 3 e 4, seção 10.4 · **Depende de:** T-140.2
...
- [X] **T-145** `P0` `M` `auditoria` Registro de KPIs por tela, no mesmo padrão do registro de painéis
...
- [X] **T-367** `P0` `M` `dados` A fonte de warehouse vive uma vez por processo, e o TLS do pooler fecha
  · **Aceite:** `obterFonteDeDados()` em modo warehouse devolve a **mesma** fonte por processo, e por isso o cache de base serve as leituras seguintes em vez de reler as dezoito views por painel (medido contra o Supabase: pagina financeira de 28 s para 0,4 s, leitura fria uma vez por instancia); uma construcao que falha nao fica memorizada; o teto de espera por conexao cobre a fila de dezoito consultas sobre tres conexoes; as ferramentas de carga e conferencia repassam `DATABASE_SSL_CA`, e a URL vai sem `sslmode`, senao o driver ignora a CA; a suite de contrato roda contra o Supabase com 768 recortes e nenhuma divergencia.
  · **PRD:** secao 10, RF-21, RF-22, D-DADOS-base-amanna · **Depende de:** T-266, T-270
- [X] **T-369** `P0` `M` `seguranca` A porta por senha, para quem apresenta entrar sem gerar link
  · **Aceite:** com `SENHA_DO_PAINEL` configurada, `/entrar` mostra um formulario que posta em `/api/entrar` (senha no corpo, nunca na URL) e a senha certa emite o **mesmo** envelope de sessao que o QR emite, com perfil de quem apresenta e dispositivo sorteado; a comparacao e de tempo constante; senha errada, vazia e ausente saem com o mesmo motivo; a rota confere a origem antes de tudo e limita seis tentativas por endereco por minuto, zerando a contagem no acerto; sem a variavel a tela volta a ser so o cartao do QR; o caminho da plateia nao muda.
  · **PRD:** secao 11, secao 13, RF-23, D-CONVITE-apresentacao · **Depende de:** T-355, T-365
- [X] **T-370** `P1` `M` `paineis` A rampa dos graficos segue a marca, e a conversa cresce no celular
  · **Aceite:** com marca aplicada, as quatro entradas de marca da rampa categorica passam a valer as cores da empresa, resolvidas em **valor literal** no servidor e recebidas por propriedade (o SVG nao resolve `var()`, e um SVG serializado perde o `:root`); as entradas de sentido — comparacao, positivo, neutro, meta — nao mudam, pela regra da secao 13; duas cores que ninguem distingue nao entram juntas: a segunda e clareada ou escurecida a partir da propria cor da empresa ate separar; sem marca, nenhum pixel muda; o grafico continua sem importar `MARCA`; no modo cheio a tipografia da conversa cresce, porque a escala era da coluna de 392 px lida de perto.
  · **PRD:** secao 5, secao 13, RF-13, D-MARCA-personalizacao · **Depende de:** T-124, T-360
- [X] **T-371** `P1` `S` `paineis` A pele do painel de referencia, no lugar do sepia do prototipo
  · **Aceite:** as 24 chaves da paleta passam a valer os tokens do painel que Produto entregou (`public/design/painel-controladoria-dreamy_1.html`), um a um; nenhum dos 369 pontos que pintam tela muda, porque toda cor ja passava por chave nomeada; quatro papeis que o arquivo de referencia empata — marca com positivo, texto com barra, comparacao com texto secundario, neutro com texto terciario — recebem valor proprio, porque cor repetida apaga a diferenca entre papeis; o contraste e remedido e reanotado: `textoTerciario` e `textoEmBarraFraco` passam a cumprir o minimo e saem da divida de H-43, e so `textoFraco` continua abaixo; os testes que mediam o **algoritmo** de ajuste deixam de usar a paleta como amostra.
  · **PRD:** secao 5, secao 13, RF-24 · **Depende de:** T-124, T-183
- [X] **T-372** `P1` `L` `paineis` Tema claro e escuro, com a paleta ativa resolvida no servidor
  · **Aceite:** a segunda paleta sai dos tokens escuros do mesmo arquivo de referencia; a moldura troca por propriedade CSS, com os seletores que o arquivo ja usa (segue o sistema, e um atributo forca); o **grafico** recebe a paleta ativa resolvida em valor literal, pelo caminho de propriedade que T-370 abriu, porque `var()` nao pinta atributo de SVG; a escolha vive num cookie, senao o servidor nao sabe o que o navegador prefere; o contraste de cada par e medido nas duas peles, e nenhuma regressao de sentido (positivo, negativo, meta) passa despercebida.
  · **PRD:** secao 5, secao 13, RF-24 · **Depende de:** T-370, T-371
- [X] **T-373** `P2` `S` `paineis` As tres familias tipograficas passam a ser pilhas de sistema
  · **Aceite:** `TIPOGRAFIA` passa a valer as pilhas do painel de referencia; o tema anterior nomeava IBM Plex e Newsreader sem nunca as carregar — nao havia `@font-face` nem link no documento —, entao o produto ja rodava em fonte de sistema sem escolher qual; nenhum arquivo novo, nenhuma requisicao antes do primeiro texto, nada que dependa de rede numa apresentacao; o caso de teste guarda a propriedade (serem de sistema), nao os nomes.
  · **PRD:** secao 5, RF-24 · **Depende de:** T-124
...
- [X] **T-429** `P0` `S` `seguranca` O convidado do QR fica no chat: tela do painel leva a entrada por senha, que oferece a volta a conversa
  · **Aceite:** com sessao valida de perfil `auditor` entrada por convite, `decidirAcesso` redireciona pagina fora de `/conversa` (e do que ela precisa) para `/entrar?motivo=plateia&ir=...` e nega `/api/*` fora da lista; a tela de entrada diz que a sessao do QR so abre a conversa e oferece "Voltar a conversa"; o layout do painel confere a mesma coisa no servidor, porque prefetch pula o proxy e o arnes `fixtures` o deixa seguir; quem apresenta continua navegando e, num navegador com sessao de plateia, digita a senha e volta a apresentar; o e2e escaneia, cadastra, abre `/rh/visao` no celular, ve a entrada e volta a conversa com o cadastro.
...
- [X] **T-438** `P0` `S` `chat` Cache de prompt para instrucao e ferramentas do laco
  · **Aceite:** o corpo do laco marca a mensagem de sistema com `cache_control` quando o modelo e da Anthropic, e o corte cobre as ferramentas junto, porque a ordem de renderizacao e `tools`, `system`, `messages`; a troca acontece so na saida para o fio, e o inspetor continua vendo texto; `Tokens` ganha `cacheLido`, lido dos dois nomes do mesmo numero (`cache_read_input_tokens` e `prompt_tokens_details.cached_tokens`); teste prova o corte com modelo Anthropic, a ausencia dele com outro, e a contagem; promovida de P2 porque e o que paga o laco por padrao (T-446).
...
- [X] **T-444** `P1` `S` `chat` A bolha desenha lista: a resposta em forma de lista vira `<ul>`, nao paragrafos com traco
  · **Aceite:** `blocosDaResposta` (antes `paragrafosDaResposta`) devolve uma uniao (`paragrafo` | `lista`), agrupa linhas consecutivas iniciadas por "- " num bloco so antes de quebrar por linha, e parte cada item no ultimo ": " entre rotulo e valor; numero negativo no inicio da linha nao vira item; `RespostaDoChat` desenha o bloco de lista como `<ul data-teste="chat-lista">` com o mesmo estilo de `Linha` do ranking; o "Traduzindo" mantem rotulo e barra de destaque (T-441); nenhum texto atual muda de aparencia.
  · **PRD:** secao 7.2, RF-13 · **Depende de:** T-441
- [X] **T-445** `P0` `M` `chat` A forma da resposta segue a pergunta: as duas instrucoes reescritas e as regras de forma separadas
  · **Aceite:** `REGRAS_DE_NUMERO` mantem as quatro regras de numero byte a byte e a regra de forma sai para `REGRAS_DE_FORMA` no mesmo modulo — sem isso nenhum prompt podia autorizar uma lista; `INSTRUCAO_DE_REDACAO` e `INSTRUCAO_DO_LACO` deixam de prescrever estrutura fixa e passam a mandar responder a pergunta na primeira frase, com as palavras dela, e a escolher a forma pelo que se pediu (numero, lista, causa, evolucao, continuacao); "Traduzindo", a comparacao com juros e o paragrafo do grafico ficam opcionais; toda frase que anuncia o que falta fica proibida, com a excecao unica de o dado que falta ser a propria resposta; `chat-instrucoes.test.ts` prova as duas propriedades nas duas instrucoes.
  · **PRD:** secao 7.1, secao 7.5, RF-15, D-CHAT-pergunta-primeiro · **Depende de:** T-433, T-444
- [X] **T-446** `P0` `M` `chat` O roteamento inverte: com gateway, o laco e o padrao e o atalho e o casamento exato
  · **Aceite:** `src/chat/rota.ts` expoe `decidirCaminho(pergunta, palpite, continuacao, temGateway)` puro e testado, que devolve `atalho` so quando ha continuacao deterministica (`herdar`), ou quando o palpite local e confiante E por termo inteiro E a pergunta nao carrega sinal composto E o resto da pergunta — tirados o vocabulario da metrica, o mes, o recorte e as palavras neutras — e vazio; `Intencao` ganha `inteiro`; `causa` continua no atalho, por nao haver ferramenta de causa; sem gateway vale o `classificar` de hoje, byte a byte, e o arnes de e2e nao muda de comportamento em caso nenhum (308 casos verdes sem uma alteracao de expectativa); o ramo `laco na duvida` (T-436) e a guarda `foraDosDados` somem, absorvidos por `sem_casamento`, e a pergunta que o laco conclui sem leitura recebe a recusa de T-441 em vez de "sem o modelo configurado"; `CHAT_ROTA=sinais` restaura o roteamento anterior; as 7 perguntas do print de Produto (2026-09-19) vao ao laco, cada uma pelo motivo certo, e as sugestoes das telas continuam no atalho salvo tres nomeadas no teste.
  · **PRD:** secao 7.1, secao 7.3, secao 13, D-CHAT-pergunta-primeiro · **Depende de:** T-435, T-438, T-445
- [X] **T-447** `P0` `S` `chat` O mes entra nas ferramentas de metrica e de serie
  · **Aceite:** `ler_metrica` e `serie_da_metrica` ganham `mes` opcional no formato `MM/AAAA`, ao nivel da ferramenta e nao dentro de `filtros` — o esquema dos cinco filtros nao muda e o teste dele continua verde; `LeituraDeMetrica` e `LeituraDeSerie` carregam `pontoDoMes`, lido da serie que a propria metrica ja traz, sem porta nova; o verificador aceita esse ponto livre, como ja aceita `pontoPedido`; o validador recusa mes fora de 01 a 12, fora da forma e de ano nao carregado; ranking e decomposicao **nao** ganham mes — o recorte por mes por dimensao fica com `consultar_dados`.
  · **PRD:** secao 7.1, secao 6.2, D-CHAT-ferramentas · **Depende de:** T-439
- [X] **T-448** `P0` `M` `chat` A checagem de relevancia: a resposta que nao responde a pergunta escala ao laco
  · **Aceite:** `src/chat/relevancia.ts` e puro e decide por tres camadas deterministicas, sem chamada de modelo — a forma da pergunta exige um tipo de evidencia (superlativo, lista e identidade exigem itens, nunca uma metrica solta), um valor em reais pedido nao volta como taxa, e ao menos um termo de conteudo da pergunta aparece no vocabulario da resposta; a forma desconta o que esta no nome da metrica, como `classificar` ja faz com os sinais; no atalho a reprovacao escala a pergunta ao laco e registra `resposta_irrelevante`; no laco a reprovacao nao bloqueia o texto, so registra; as tres respostas do print reprovam por `sem_itens` e as sugestoes que ficam no atalho aprovam, por teste.
  · **PRD:** secao 7.1, secao 7.7, RF-15, RF-16 · **Depende de:** T-446
- [X] **T-449** `P0` `L` `dados` O esquema `amanna_chat` e o papel que so o enxerga
  · **Aceite:** `ferramentas/dados/sql/012_chat_sql.sql` cria o esquema, o papel `amanna_chat_ro` (NOLOGIN, com `default_transaction_read_only`, `statement_timeout`, `idle_in_transaction_session_timeout` e `search_path` sem `amanna`), as revogacoes de `set_config`, da familia `query_to_*` e de `pg_sleep` em PUBLIC com os GRANTs devolvidos as roles do Supabase dentro de um `DO` guardado por `pg_roles`, a funcao `no_escopo` (STABLE e fail-closed) e as views sobre o detalhe — razao lancamento a lancamento, marcas de qualidade por lancamento, folha e cadastro por pessoa, titulos, caixa e dimensoes; `ponto_ausencias` (CID), `pesquisa_engajamento` e `candidaturas` ficam fora; `fora_do_padrao` vira funcao de janela e `em_conta_parada` vira CTE por conta e mes, porque a forma de 007 paga por lancamento estoura o tempo; `chat-sql-migracao.test.ts` aplica 001→012 em PGlite **sem CSV** e prova que nenhuma coluna proibida existe em `amanna_chat`, que `amanna.*` e negado ao papel, que a escapada por `set_config` + `query_to_xml` morre na revogacao, que o escopo e fail-closed e que toda view com entidade ou area cita `no_escopo`.
  · **PRD:** secao 7.5, secao 11, secao 10.1, D-CHAT-sql · **Depende de:** T-270
- [X] **T-450** `P0` `M` `dados` A porta da consulta: conexao propria, embrulho e escopo por GUC
  · **Aceite:** `cliente-chat.ts` abre um pool proprio (`max: 2`) sobre `DATABASE_URL_CHAT`, sem importar `pg` — e a conexao separada, e nao `SET LOCAL ROLE`, que e a defesa; `consulta-livre.ts` embrulha o SQL do modelo em `SELECT * FROM (\n<sql>\n) AS resultado LIMIT $1`, com `SET TRANSACTION READ ONLY`, os tetos e as tres GUCs de escopo postas pelo comando `SET`; `sql-guarda.ts` e um lint cujo cabecalho abre dizendo que **nao e a defesa**, e recusa o que nao comeca em SELECT/WITH, o `;` do meio e uma lista de funcoes, ignorando o que esta dentro de aspas e de comentario; `src/acesso/consulta.ts` e a unica porta que `src/chat/` importa, e monta sessao, escopo e modulo antes do banco; `DATABASE_URL_CHAT` entra no esquema de configuracao e no `.env.example`.
  · **PRD:** secao 7.5, secao 11, D-CHAT-sql · **Depende de:** T-449
- [X] **T-451** `P0` `M` `dados` O resultado vira envelope, com a formatacao do nosso lado
  · **Aceite:** `formatarReais` e `formatarNumero` entram em `formato.ts`, o modulo cuja regra declarada e que arredondamento nao acontece em outro lugar; a unidade de cada coluna sai do dicionario semeado em `amanna_chat` (e nao de `information_schema`, que reordena e derrubaria o cache da secao 7.4); `chat/ferramentas/consulta.ts` monta a leitura com as celulas ja escritas, o rotulo de cada linha e os `NumeroPermitido` — celula livre so no resultado 1×1, com rotulo por perto em qualquer outro; `chat-sql-envelope.test.ts` prova a ida e a volta pelo `divergencias`: a celula com o rotulo passa, a mesma solta nao passa, um numero que a consulta nao devolveu nao passa, e a soma de duas linhas tambem nao.
  · **PRD:** secao 7.1, RF-15, D-CHAT-sql · **Depende de:** T-450
- [X] **T-452** `P1` `S` `chat` O inspetor deixa passar pessoa por nome, cargo, area e custo
  · **Aceite:** `CAMPO_DE_PESSOA` para de barrar `colaborador`, `nome_do_colaborador` e `funcionario` — que matariam em silencio a primeira consulta com coluna de gente — e passa a barrar, por nome de campo, CPF, matricula, nascimento, conta bancaria, PIX, sindicato, RG, PIS e titulo de eleitor; CPF e e-mail continuam barrados **por forma**, em qualquer campo; teste de tabela cobre os dois lados; a decisao de Produto (2026-09-19) fica citada no comentario do modulo.
  · **PRD:** secao 11, D-CHAT-sql · **Depende de:** T-352
- [X] **T-453** `P0` `M` `chat` A costura: `consultar_dados` entra no vocabulario como nona ferramenta
  · **Aceite:** `consultar_dados` entra em `NOMES_DE_FERRAMENTA`, no validador, no executor e em `PedidoValidado`, e so e oferecida com `DATA_SOURCE=warehouse` **e** `DATABASE_URL_CHAT` presente — em fixtures o modelo nem a enxerga, e o arnes de e2e nunca a alcanca; `LeituraDeConsulta` entra em `LeituraDeFerramenta` e o compilador nomeia os seis switches nao exaustivos (`numerosDe`, `fraseDe`, `paraOModeloLeitura`, `executarPedido`, `fraseDePasso`, `TITULO_DA_LEITURA`/`CorpoDaLeitura`); a descricao da ferramenta diz **quando nao usa-la** (tente o ranking da dimensao antes); o esquema vai na mensagem do usuario, e nao na de sistema, para nao mexer no corte de cache nem no que o inspetor confere; o cabecalho de `catalogo.ts`, que afirmava "nao ha SQL", passa a dizer o que mudou e por que.
  · **PRD:** secao 7.1, secao 11, RF-15, D-CHAT-sql · **Depende de:** T-451, T-452
- [X] **T-454** `P1` `M` `chat` A resposta sem metrica do catalogo: sentinela, texto montado, previa e rodape
  · **Aceite:** o laco que conclui so com leitura de consulta monta uma resolucao sentinela (`metrica: ""`) em vez de cair na recusa "sem o modelo configurado"; a sentinela custa quatro guardas nomeadas — a rota nao emite previa, as sugestoes caem no guia da tela, `montarTexto` para na leitura, e o rodape diz `consulta ao banco · N linhas · fonte …`; um `<details>` "consulta registrada" mostra o SELECT executado, que e o que o principio P3 exige de todo numero; `montarTexto` passa a juntar blocos por linha em branco e deixa de escrever `Formula:`, `Referencias:` e `Sem comparacao com juros:`, que a tela ja mostra ou que a decisao proibe; `TurnoAnterior` ganha `assunto`, sem numero e cortado em `lerPedido`, para "E esses lancamentos sao o que?" nao perder o fio depois de uma resposta de consulta.
  · **PRD:** secao 7.2, secao 9.4, RF-13, RF-15 · **Depende de:** T-444, T-453
- [X] **T-455** `P0` `S` `chat` No prototipo, a consulta liga sem provisionar papel, e o esquema abre o resto do dado
  · **Aceite:** Produto (2026-09-19): "nao precisa disso, pode deixar todos consultarem os dados do banco, e so um prototipo e os dados sao falsos de mockup"; `DATABASE_URL_CHAT` vira opcional e, sem ela, a consulta usa a `DATABASE_URL` de sempre — a capacidade liga sozinha com `DATA_SOURCE=warehouse`; o esquema ganha ausencias (com CID), engajamento (com comentario aberto), vagas, candidaturas, treinamento, horas por projeto, movimentacao, notas de saida, orcamento, projetos, emprestimos, metas e as dimensoes de cliente, fornecedor e cargo, e o cadastro de pessoa passa a levar nascimento, genero, escolaridade, sindicato, banco e motivo de desligamento; so o CPF fica fora, e por razao funcional — o inspetor de saida barra resultado com forma de CPF, e expor a coluna mataria o laco em silencio; a migracao derruba as views antes de recriar, porque `CREATE OR REPLACE VIEW` so aceita acrescentar coluna no fim; a descricao da ferramenta deixa de dizer "ultima opcao" e passa a dizer que a consulta alcanca tudo; `chat-sql-migracao` prova a lista curta de proibidas e `tests/dados` prova as views novas sobre a base real.
  · **PRD:** secao 7.5, secao 11, D-CHAT-sql · **Depende de:** T-453
- [X] **T-457** `P0` `M` `chat` O laco volta a funcionar: o search_path da consulta, o que foi lido para de ser descartado, e os agregados abrem
  · **Aceite:** medido contra o gateway e o banco reais, nao suposto — as duas primeiras hipoteses cairam (os quatro corpos com `cache_control` voltam 200; a rodada 1 leva 3,2 s) e a terceira e a causa: `relation "lancamento" does not exist`, porque a instrucao ensina `FROM lancamento` e o `search_path` vinha do `ALTER ROLE`, que so vale pela conexao do papel restrito — `SET LOCAL search_path` entra na transacao de `consulta-livre.ts` e vale para as duas conexoes; `conversarComFerramentas` devolve `{texto: null, parada: "falhou"}` quando ja houve leitura, em vez de `null`, e o estagio 3 redige do envelope (o registro mostrava `laco_falhou` com `leituras: 3` jogadas fora); o enum dos 145 ids sai dos seis esquemas e a lista vai uma vez no contexto, de 27.483 para 6.772 caracteres, com teto em teste; a migracao 013 abre os agregados mensais (`fin_mes`, `rh_mes`, `dre_conta_mes` e mais dez), e o dicionario ganha os apelidos de coluna calculada, sem os quais o valor saia sem "R$"; `mesesDaPergunta` reconhece mais de um mes e a rota manda "abril e agosto" ao laco; a instrucao do laco ganha quatro exemplos de consulta resolvidos e a regra de dois periodos; `ferramentas/chat/perguntar.mts` roda a lista de perguntas contra o chat de verdade e imprime caminho, autoria e tempo.
  · **PRD:** secao 7.1, secao 7.4, RF-15, D-CHAT-sql · **Depende de:** T-453
- [ ] **T-458** `P1` `S` `chat` O teste vivo com chave, e o papel restrito quando o banco for de cliente
  · **Feito:** `chat-vivo.test.ts` ja tem as duas perguntas do print, a asercao de que toda resposta composta leu alguma coisa, e o caso que prova que nenhuma resposta anuncia o que falta; sem chave o ficheiro salta.
  · **Falta:** rodar `CHAT_VIVO=1 OPENROUTER_API_KEY=… npx vitest run tests/unidade/chat-vivo.test.ts` e conferir `cache_read_input_tokens > 0` na segunda composta. E, quando o banco deixar de ser de mockup: provisionar a senha de `amanna_chat_ro`, montar `DATABASE_URL_CHAT` e reduzir o esquema `amanna_chat` ao que o contrato com o cliente permitir — o desenho do papel ja esta pronto na migracao 012.
  · **PRD:** secao 7.7, O4 · **Depende de:** T-448, T-453
- [X] **T-456** `P1` `S` `docs` As decisoes registradas e o PRD corrigido
  · **Aceite:** `D-CHAT-sql.md` registra a reversao das duas frases da secao 7.5, a escapada medida por `set_config` + `query_to_xml` e por que a defesa e a conexao separada, o que fica fora do esquema e por que, o escopo por GUC e o envelope; `D-CHAT-pergunta-primeiro.md` registra o diagnostico medido (6 de 7 perguntas naturais sem sinal), o roteamento invertido com os numeros de custo com e sem cache, a forma livre e a checagem de relevancia; as secoes 7.5 e 11 do PRD apontam para as duas; `D-CHAT-ferramentas` e `D-CHAT-resposta-completa` ganham a linha de "alterado por"; `npm run instrucoes` verde.
  · **PRD:** secao 9.4, secao 18 · **Depende de:** T-446, T-448, T-454
