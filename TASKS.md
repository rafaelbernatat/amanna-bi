...
- [ ] **T-455** `P1` `S` `chat` O conjunto de avaliacao das perguntas reais, rodado com chave e contra o banco
  · **Feito:** `chat-vivo.test.ts` ja ganhou as duas perguntas do print, a asercao de que toda resposta composta leu alguma coisa, e o caso que prova que nenhuma resposta anuncia o que falta; sem chave o ficheiro continua a saltar.
  · **Falta (precisa de chave e do Supabase):** rodar `CHAT_VIVO=1 OPENROUTER_API_KEY=… npx vitest run tests/unidade/chat-vivo.test.ts`; provisionar a senha de `amanna_chat_ro` e `DATABASE_URL_CHAT`; rodar o roteiro manual de D-CHAT-sql contra o banco real — as seis perguntas, a conferencia de que nenhum CPF, nascimento, banco ou sindicato sai nem quando pedido de proposito, e `cache_read_input_tokens > 0` na segunda composta.
