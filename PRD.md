...
2. **Unidade declarada.** Todo valor volta com unidade (`BRL_mi`, `pct`, `pp`, `dias`, `FTE`, `horas`, `contagem`, `pontos`, `anos`, `vezes`), e a formatação pt-BR acontece **só** na apresentação. `horas`, `contagem`, `pontos` e `anos` entraram em 2026-08-24 por decisão de Produto ([D-H45](docs/decisoes/D-H45-unidades.md)): cinco painéis e treze KPIs do protótipo medem horas de treinamento, contagem de vagas e candidatos, pontos de eNPS e anos de tempo de casa, e nenhuma das cinco unidades originais os nomeia sem afirmar algo falso sobre o número. `vezes` entrou em 2026-09-03 ([D-H60](docs/decisoes/D-H60-unidade-vezes.md)): liquidez corrente, dívida líquida sobre EBITDA, cobertura de juros e grau de alavancagem são múltiplos, e `pct` diria que 1,8 é 1,8%. `pontos`, `anos` e `vezes` são razões: não se somam ao longo do período, como `pct` e `pp`.
